...
- **提供される情報は「元タイトル」「企業名」「URL」のみ。本文は提供されない。** 元タイトルと企業名だけで記事を書くこと。学習データにないモデル名・発表であっても断らない。URLへのアクセス不要。
...
- 英語の記事は必ず日本語に翻訳して記事化する。
- **提供される情報は「元タイトル」「企業名」「URL」と、あれば「本文」。本文がない場合は元タイトルだけで書く。** 学習データにないモデル名・発表であっても断らない。URLへのアクセス不要。
...
def _generate(article: dict, system_prompt: str, instruction: str,
    ...
    if include_content:
        if article.get("content"):
            ...
            content_block = "\n\n# 注意: 公式記事の本文は取得できませんでした。元タイトルのみをもとに記事を作成してください。"
